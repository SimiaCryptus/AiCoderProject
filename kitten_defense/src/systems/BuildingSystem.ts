...
interface BuildingState {
    buildings: Building[];
    templates: Record<BuildingType, BuildingTemplate>;
    addBuilding: (type: BuildingType, position: Vector3, owner: string) => Building | null;
    removeBuilding: (id: string) => void;
    upgradeBuilding: (id: string) => boolean;
    addModule: (buildingId: string, moduleType: ModuleType, position: Vector3) => boolean;
    ...
    canBuild: (type: BuildingType, position: Vector3, owner: string) => boolean;
}

const buildingTemplates: Record<BuildingType, BuildingTemplate> = {
...
export const useBuildingStore = create<BuildingState>((set, get) => ({
    ...
    addBuilding: (type: BuildingType, position: Vector3, owner: string) => {
        const template = buildingTemplates[type];
        if (!template || !get().canBuild(type, position, owner)) return null;
    ...
    addModule: (buildingId: string, moduleType: ModuleType, position: Vector3) => {
    ...
    repairBuilding: (id: string, amount: number) => {
        set(state => ({
            buildings: state.buildings.map(b => {
    ...
    getBuildingsByOwner: (owner: string) => {
    ...
    canBuild: (type: BuildingType, position: Vector3, owner: string) => {
        const template = buildingTemplates[type];
        ...
        if (template.requirements.territory) {
            const gameStore = useGameStore.getState();
            const territory = gameStore.gameState.territories.find(t =>
                Math.abs(t.position.x - position.x) < 10 &&
                Math.abs(t.position.z - position.z) < 10
            );
            if (!territory || territory.owner !== owner) return false;
        }

        // Check building collisions
        const buildings = get().buildings;
        const collision = buildings.some(b =>
            Math.abs(b.position.x - position.x) < 5 &&
